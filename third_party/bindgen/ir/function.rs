...
impl quote::ToTokens for ClangAbi {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        match *self {
            ...
            // wreq-php patch: PHP 8.5's tail-call VM exposes items with the
            // `preserve_none` calling convention (CXCallingConv 20), which this
            // bindgen release does not recognize and would otherwise panic on.
            // We never call those VM internals, so emitting the C ABI for an
            // unknown convention is safe and lets PHP 8.5 build.
            Self::Unknown(_cc) => {
                let abi = "C";
                tokens.append_all(quote! { #abi });
            }
